...
TEST_F(DispatcherTest, AppIdTest)
{
	...
	EXPECT_STREQ("foobar_baz_1.2.3", lomiri_app_launch_mock_get_last_app_id());
	lomiri_app_launch_mock_clear_last_app_id();
	...
	dispatcher_send_to_app(out_appid, out_url);
	EXPECT_STREQ("container-id_org.canonical.app1_0.0", lomiri_app_launch_mock_get_last_app_id());
...
TEST_F(DispatcherTest, ApplicationTest)
{
	...
	dispatcher_send_to_app(out_appid, out_url);
	ASSERT_STREQ("foo", lomiri_app_launch_mock_get_last_app_id());
	lomiri_app_launch_mock_clear_last_app_id();

	/* No .desktop */
	dispatcher_url_to_appid("application:///foo", &out_appid, &out_url);
	dispatcher_send_to_app(out_appid, out_url);
	ASSERT_TRUE(nullptr == lomiri_app_launch_mock_get_last_app_id());
	lomiri_app_launch_mock_clear_last_app_id();
	...
	dispatcher_send_to_app(out_appid, out_url);
	ASSERT_STREQ("my-really-cool-app", lomiri_app_launch_mock_get_last_app_id());
	lomiri_app_launch_mock_clear_last_app_id();

	/* Good Click Style */
	dispatcher_url_to_appid("application:///com.test.foo_bar-app_0.3.4.desktop", &out_appid, &out_url);
	dispatcher_send_to_app(out_appid, out_url);
	ASSERT_STREQ("com.test.foo_bar-app_0.3.4", lomiri_app_launch_mock_get_last_app_id());
	lomiri_app_launch_mock_clear_last_app_id();
...
TEST_F(DispatcherTest, CalendarTest)
{
	...
	dispatcher_send_to_app(out_appid, out_url);
	ASSERT_STREQ("com.ubuntu.calendar_calendar_9.8.2343", lomiri_app_launch_mock_get_last_app_id());
	lomiri_app_launch_mock_clear_last_app_id();
...
{
    auto mock = dbus_test_dbus_mock_new("com.lomiri.LomiriDash");

    dbus_test_task_set_name(DBUS_TEST_TASK(mock), "LomiriDash");
