...
TEST_F(OverlayTrackerTest, AddOverlay) {
	...
	EXPECT_STREQ("url-overlay", lomiri_app_launch_mock_last_start_session_helper);
	EXPECT_STREQ("app-id", lomiri_app_launch_mock_last_start_session_appid);
	EXPECT_STREQ("http://no-name-yet.com", lomiri_app_launch_mock_last_start_session_uris[0]);

	delete tracker;

	EXPECT_STREQ("url-overlay", lomiri_app_launch_mock_last_stop_helper);
	EXPECT_STREQ("app-id", lomiri_app_launch_mock_last_stop_appid);
...
TEST_F(OverlayTrackerTest, OverlayABunch) {
	...
	for (auto name : std::vector<std::string>{"warty", "hoary", "breezy", "dapper", "edgy", "feisty", "gutsy", "hardy", "intrepid", "jaunty", "karmic", "lucid", "maverick", "natty", "oneiric", "precise", "quantal", "raring", "saucy", "trusty", "utopic", "vivid", "wily"}) {
		...
		EXPECT_EQ(name, lomiri_app_launch_mock_last_start_session_appid);
	}
}
...
ualStop (const char * helper_type, std::function<bool(OverlayTrackerMir *)> addFunc)
{
	...
	lomiri_app_launch_mock_observer_helper_stop_func("app-id", "instance", helper_type, lomiri_app_launch_mock_observer_helper_stop_user_data);
...
mirStop (const char * helper_type, const char * appid, std::function<bool(OverlayTrackerMir *)> addFunc)
{
	...
	EXPECT_STREQ(helper_type, lomiri_app_launch_mock_last_stop_helper);
	EXPECT_STREQ(appid, lomiri_app_launch_mock_last_stop_appid);
	EXPECT_STREQ("instance", lomiri_app_launch_mock_last_stop_instance);
}
